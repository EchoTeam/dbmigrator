CREATE OR REPLACE FUNCTION ItemAdd_v2(
...
BEGIN
	FOR NewTag IN SELECT UNNEST(NewItemTags) LOOP
		BEGIN
			INSERT INTO Tags (ItemURI, ItemExcKey, ItemTag)
			VALUES (NewItemURI, NewItemExcKey, NewTag);
		EXCEPTION
			WHEN UNIQUE_VIOLATION THEN
		END;
	END LOOP;
	FOR NewMarker IN SELECT UNNEST(NewItemMarkers) LOOP
		BEGIN
			INSERT INTO Markers (ItemURI, ItemExcKey, ItemMarker)
			VALUES (NewItemURI, NewItemExcKey, NewMarker);
		EXCEPTION
			WHEN UNIQUE_VIOLATION THEN
		END;
	END LOOP;
...
GRANT EXECUTE ON FUNCTION ItemAdd_v2(
	Items.ItemURI%TYPE,
